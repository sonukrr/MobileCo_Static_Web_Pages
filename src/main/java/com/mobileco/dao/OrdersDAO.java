...
public class OrdersDAO {
	...
	public List<Orders> getAllOrders(Customer customer)
	{
		String query="Select o.id,o.date_ordered,o.date_expected,o.date_delivered,o.quantity, "
				+ "o.product_id,m.name as product,b.name as brand,p.brand_id, p.price "
						+ "from orders o "
						+ "inner join brand b "
						...
						+ "and p.mobile_id=m.id "
						+ "and p.brand_id=b.id "
						+ "and o.customer_id=c.id "
						+ "where c.id=? "
						+ "union "
						+ "Select o.id,o.date_ordered,o.date_expected,o.date_delivered,o.quantity,"
						+ "o.product_id,a.name as product,b.name as brand,p.brand_id, p.price "
								+ "from orders o "
								+ "inner join brand b "
								+ "inner join customer c "
								+ "inner join product p "
	...
	public void placeOrder(List<Cart> cartItems, Customer customer) throws MobilecoException{
		...
		try{
			...
			for(Cart itemInCart : cartItems){
				...
				psmt.executeUpdate();
				productDao.updateQuantity(connection,
						itemInCart.getProduct().getId(), 
						itemInCart.getOrderQuantity());
			}
