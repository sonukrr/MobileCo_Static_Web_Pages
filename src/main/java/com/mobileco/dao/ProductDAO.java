...
public class ProductDAO {
	...
	public List<Product> getAllProducts(String searchStr) throws MobilecoException{
		...
		String query = "select * from product p "
				+ "left join accessories a on a.id = p.accessories_id "
				+ "left join mobile m on m.id = p.mobile_id "
				+ "join brand b on b.id = p.brand_id "
				+ "where a.name like '"+searchStr+"%' or m.name like '"+searchStr+"%' or "
				+ "b.name like '"+searchStr+"%' or m.ram = ? or m.rom = ? or m.fcamera=? or m.rcamera=?"
	...
	public List<Product> getMobiles() throws MobilecoException{
		...
		try{
			...
			while(resultSet.next()){
				...
				Product product = new Product();
				Mobile mobile = new Mobile(mobileId,
								mobileName,
								ram,
								rom,
								fcamera,
